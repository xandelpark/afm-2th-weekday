...
- 소제목·이모지 나열은 쓰지 마라. 본문은 자연스러운 문단으로 쓰되, 아래 '칭찬 포인트 3가지'만 예외로 1. 2. 3. 번호를 매겨 넣어라.
...
[칭찬 포인트 3가지 — 카페·블로그 공통, 반드시 포함]
- 본문 도입 문단 뒤에, 예비신부(예비고객)에게 가장 먹힐 만한 마리안웨딩의 강점 3가지를 1. 2. 3. 번호로 정리해라.
- 반드시 신부가 설문에서 체크한 내용에 근거해서 뽑아라(없는 장점 창작 금지).
- 각 항목은 한두 줄, 광고 문구가 아니라 실제 후기처럼 구체적으로. (예: "1. 억지 연출 없이 자연스러운 사진 — 포즈 잡는 티가 안 나서…")
- 3가지 앞에 "제가 느낀 장점 3가지만 꼽자면요," 정도의 짧은 도입 한 줄을 붙여도 좋다.
- 번호 목록 뒤에는 다시 자연스러운 마무리 문단으로 이어가라.
...
[제목 작성 규칙 — SEO와 어그로(훅) 둘 다 반드시]
제목은 "SEO 키워드(앞) + 어그로 훅(뒤)" 구조로 만들어라.
1) SEO — 제목에 검색 키워드를 반드시 넣어라:
   앞부분에 (지역 또는 예식장명) + 상품키워드(본식스냅 / 본식스냅DVD / 본식DVD) + "후기",
   그리고 업체명 "마리안웨딩"도 제목 안에 반드시 자연스럽게 넣어라(브랜드 검색 노출용).
   예) "서울 본식스냅 후기, 마리안웨딩 3군데 비교하고 정한 이유", "마리안웨딩 그랜드워커힐 본식스냅DVD 후기 …".
   ※ 비제휴라 예식장 이름을 못 쓰면 지역만(예식장명 금지). "마리안웨딩"은 어떤 경우에도 제목에 넣어라.
2) 어그로 훅 — 그 뒤에 클릭하고 싶게 만드는 훅을 붙여라. 아래 검증된 훅 공식 중 콘텐츠에 맞는 걸 골라 자연스럽게:
   · 정보격차/궁금증: "…고민이라면 이거 보고 결정하세요"
   · 반전/결과: "반신반의했는데 결과 보고 놀란…", "…할 뻔"
   · 솔직·리얼: "내돈내산 찐후기", "가감없는 솔직후기"
   · 손실회피: "이거 모르고 계약할 뻔", "안 봤으면 후회했을…"
   · 구체 디테일/숫자: "3군데 비교하고 여기로 정한 이유"
   · 감정 극대화: "사진 받고 남편이랑 감탄한…", "울컥했던…"
   · 타겟 호명: "예신 필독", "본식스냅 고민 예신 주목"
3) 길이 30~45자. 이모지 남발·허위·과장광고 금지. 실제 신부가 쓴 후기 제목처럼.

[출력 형식 — 반드시 이 형식]
제목: <위 규칙대로: SEO 키워드 + 어그로 훅>
