...
> **커버 제목은 항상 후킹 공식([[dailytennis-hook-titles]])으로.** 캡션뿐 아니라 **커버(썸네일) 한글 제목**도 설명형 금지 — FOMO 손실회피("~하면 손해", "모르면 후회") 1순위. 단 **자극은 카피로만, 디자인은 매거진 유지**(빨강 클릭베이트 펄 금지). 영문 히어로(Fraunces italic+마침표)는 매거진 톤, 한글 훅(NSR 800)이 후킹을 담당. (2026-07-05 슬기 "가능하면 후킹공식으로 제목을".)
...
## STEP 5 — 저장 & 보고
- **저장 위치(`dailytennis-output-location`)**: 작업 파일(빌더·프레임·소스이미지)은 프로젝트 폴더(영상 있으면 원본 영상 폴더, 이미지 수급형이면 `~/Downloads/<TOPIC>/`).
  - **⭐ 최종 산출물(합성 릴스 `.mp4` + 동명 캡션 `.txt`)은 `~/Downloads/<콘텐츠명>/` 전용 폴더를 만들어 그 안에 넣는다** — 루트에 파일 흩뿌리기 금지. (2026-07-05 지침 "다운로드에 바로 넣어줘" → "폴더를 만들고 그 안에 넣어줘야지".) 폴더명은 한글 콘텐츠명(예: `데일리테니스_라켓선택가이드`). 빌더·프레임·소스 등 작업파일은 프로젝트 폴더에 그대로, 전용 폴더엔 최종본만.
