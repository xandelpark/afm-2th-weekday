...
> **⭐ 배색 최종 확정 (2026-07-01 · 슬기 "지금 인스타 피드 톤앤무드에 맞게"):** 정답은 **라이트 피드 톤** — **크림/베이지/연한 라벤더 배경 사이클 + 코트 퍼플 `#5B3F73` 강조 + 딥 골드 키커 + 차콜 잉크 텍스트**. **딥 플럼 `#2E1F3D`·차콜 `#1A1612` 다크 배경은 "진하다"고 반려됨 → 라켓/제품 누끼 나열도 다크 말고 라이트 배경(소프트 섀도우)으로.** 다크는 극히 제한적 악센트로만(도배 금지). 정답 배색 레퍼런스: `~/Downloads/dailytennis_5brands/build_5brands.py`.
...
- **제품 사진(흰배경 제품샷)** = 정면뷰만 크롭 → **외곽 흰배경 누끼**(PIL `ImageDraw.floodfill` 코너 시드, `thresh~30`; **흰색 제품은 `thresh~12~15`로 낮춰** 프레임이 안 지워지게) → 자동 크롭 → **라이트 배경 슬라이드**(크림/베이지/라벤더)에 소프트 `drop-shadow`로 띄움(§컬러 배색 확정). 스트링 베드(내부 흰색)는 그대로 둬 제품샷처럼 보이게.

### ⚠️ 인물 사진 커버/썸네일 & 의상·장비 콜아웃 박스 (사람 가림/잘림 금지 — 슬기 반복 지적)
- **인물 사진을 첫 썸네일(커버)로 쓸 때 얼굴 잘림 금지.** 세로 인물을 **랜드스케이프(가로) 박스**에 넣으면 이마/턱이 잘린다 → 프레임을 **인물 세로 비율에 맞춘 세로(portrait) 박스**로 잡아 **얼굴~상반신이 온전히** 들어오게. cover-fit은 최소 스케일이라 세로 여유가 없으면 무조건 크롭되므로, 박스 aspect를 인물 비율(≈0.75~0.8)에 근접시키고 `fy`로 머리 위 여백을 확보. 빌드 후 **반드시 눈으로 확인**(얼굴 전체+어깨 보이는지).
- **의상/장비 셋업을 콜아웃 박스(리더선)로 표시할 때 박스가 인물을 가리지 않게** — 박스는 **여백(코너·사이드)**에 두고 **리더선으로만** 아이템을 지목한다. 박스로 얼굴/몸통을 덮거나, 인물이 프레임 밖으로 잘리면 안 됨. 9:16 재렌더 시 인물 위치가 바뀌므로 **앵커 좌표를 새 프레임에 맞춰 재배치**하고 역시 눈으로 확인.
- 요약: **주인공(인물)은 절대 가리거나 자르지 않는다.** 텍스트·카드·박스는 인물을 피해서 얹는다.
...
- **A 슬라이드(라이트, 크림/베이지/라벤더 §배색)**: 라켓 누끼 우측 풀하이트 + 좌측 `브랜드(이탤릭)·모델(Black Han Sans)·연식·한줄·스펙표(JetBrains Mono)·특징 3`. 브랜드별 액센트 컬러 stripe + 뒤 거대 페이드 브랜드 이니셜.
