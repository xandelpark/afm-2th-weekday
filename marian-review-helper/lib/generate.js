...
- 아래 'AI 후기 티'를 특히 피해라 (이게 제일 인위적으로 읽힌다):
  · 시작할 때 무엇을 쓸지 예고하지 마라 ("자세히 적어볼게요", "결론부터 말씀드리면", "먼저 ~부터"). 바로 이야기로 들어가라.
  · 첫 '문장'에 지역+예식장+상품키워드를 몰아넣지 마라. 말문은 자연스럽게 열고, 키워드는 한 박자 뒤 첫 문단 안에서 문맥으로 흘려라.
  · 마무리를 매번 "같은 고민 하는 예비신부님들께 도움이 되셨으면"류로 끝내지 마라 — 가장 뻔한 AI 엔딩이다. 담백한 개인적 한마디로 맺거나, 억지로 정리하지 말고 자연스럽게 끊어라.
  · 모든 문단을 교훈·소감으로 예쁘게 매듭짓지 마라. 어떤 문단은 사실만 적고 툭 끝나도 된다.
  · "몽글몽글", "감동이 배가 되더라고요", "그날로 돌아간 기분" 같은 감성 클리셰를 쓰지 마라. 감정은 구체적 상황으로 보여줘라(직접 감정을 설명하지 말고).
  · 줄표(—)로 부연을 삽입하는 습관을 쓰지 마라.
  · 매끄럽게 완벽할 필요 없다. 시시콜콜하고 사소한 실제 디테일 한두 개를 넣어 진짜 사람이 쓴 느낌을 줘라.
...
- '비교했던 다른 업체' 항목은 반드시 후기에 반영해라:
  · 다른 업체를 절대 깎아내리거나 단점을 쓰지 마라(비하·부정 금지 — 타 업체 피해 방지). 오직 "여러 곳과 견줘본 끝에 마리안웨딩이 저희와 가장 잘 맞았다"는 긍정 프레임으로만.
  · 업체 이름이 있으면 본문에 자연스럽게 언급해도 된다. '없음/없다/안 함' 같은 답이면: 다른 곳과 비교하지 않고 처음부터 마리안웨딩만 보고 확신해서 선택했다는 뉘앙스로 녹여라.
  · 반영 분량은 글 유형을 따라라 — 사진후기는 한 문장으로 가볍게, 계약후기는 비교·선택 과정을 좀 더 구체적으로(아래 사용자 메시지의 '계약후기 특별 지침' 참고).
...
- 번호 목록 뒤 짧은 마무리 문단으로 담백하게 맺어라 (매번 '예비신부님께 도움이 되길'류 상투 엔딩으로 끝내지 말 것).`,
...
- 마무리는 담백하게 개인적 소감 정도로 (상투적인 '도움이 되길' 엔딩 지양).`,
...
- 마지막 문단은 종합 소감 정도로 담백하게 (권유성 상투 문구 지양).`,
];

// 계약후기 전용 문단 구성 — '비교→선택 이유'가 핵심이라 사진후기 구성과 다르다.
const CONTRACT_STRUCTURES = [
  `[이번 글 구성 — 업체 비교·선택 과정 중심 (계약후기)]
- 도입(첫 문단에 지역+상품키워드): 본식스냅을 알아보게 된 계기와 "어디로 정할지" 고민.
- 비교 과정 문단: 몇 군데를 어떤 기준으로 비교했는지(입력된 비교 업체가 있으면 이름을 자연스럽게 언급), 그중 마리안웨딩이 왜 저희와 맞았는지. (타 업체 비하 절대 금지)
- 계약 결정 이유 문단: 체크한 '결정 이유'를 각각 왜 중요했는지까지 구체적으로.
- 상담 인상 + 마무리: 상담에서 느낀 점을 담백하게 (상투적 '도움이 되길' 엔딩 지양).`,

  `[이번 글 구성 — '이걸로 정한 이유' 번호형 (계약후기)]
- 도입(지역+상품키워드 + 비교 고민): 여러 곳 알아보던 상황.
- "제가 마리안웨딩으로 정한 이유 3가지," 같은 짧은 도입 + 1. 2. 3. 번호 목록 (체크한 결정 이유 근거, 각 항목 두세 줄로 충실히). 비교 업체가 있으면 도입이나 마무리에서 "○○ 등과 견줘봤지만"으로 자연스럽게.
- 번호 뒤 상담 인상 정도로 담백하게 마무리 (권유성 상투 문구 지양).`,

  `[이번 글 구성 — 예비신부 결정 가이드형 (계약후기)]
- "본식스냅 어떻게 고르지?" 고민하는 예비신부 관점.
- 도입(지역+상품키워드): 비교하며 세운 나만의 기준(가격·구성·스타일·후기 등 체크한 것).
- 그 기준으로 볼 때 마리안웨딩이 어땠는지 문단별로. 비교 업체는 "○○, ○○도 봤는데"로 사실만 담백하게.
- 같은 고민 하는 예비신부에게 도움 되는 팁 + 마무리.`,
];

// 계약후기 특별 지침 — 더 자세히 + 비교 살리기 + 선택·비교 SEO
const CONTRACT_GUIDE = `[계약후기 특별 지침 — 더 자세하고 비교·선택 과정이 살아나게]
- 이 글의 핵심은 '왜 마리안웨딩을 선택했는가'다. 비교 과정과 결정 이유를 사진후기보다 더 구체적으로, 충실히 풀어라.
- 입력된 '비교했던 다른 업체'가 있으면 그 이름을 본문에 자연스럽게 언급하되, 절대 깎아내리지 말고 "여러 곳과 견줘본 끝에 마리안웨딩이 가장 잘 맞았다"는 긍정 프레임으로만. (예비신부가 "○○ 후기" 검색으로 유입되는 SEO 효과도 있다)
- 결정 이유(체크한 항목)는 나열에 그치지 말고 각각 '왜 저희에게 중요했는지'까지 풀어라.
