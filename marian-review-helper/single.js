...
app.post("/api/access", (req, res) => require("./api/access")(req, res));
app.get("/api/approve", (req, res) => require("./api/approve")(req, res));
...
async function start() {
  if (process.env.SKIP_DB !== "1") {
    try {
      await db.ensure();
      console.log("✓ Supabase 연결·테이블 준비 완료 (marian_review.usage)");
    } catch (e) {
      console.warn("⚠️  DB 준비 실패:", e.message, "(SKIP_DB=1 로 우회 가능)");
    }
  }
  app.listen(PORT, () => {
    console.log(`마리안웨딩 후기 dev 서버: http://localhost:${PORT}`);
